    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($xpros)
            ...
            ->addColumn('ps_re_hi', function ($row) {
                return $row->re_hi != 0 ? round($row->ps_hi / $row->re_hi, 2) : 'N/A';
            })
            ->addColumn('ps_pi_hi', function ($row) {
                return $row->pi_hi != 0 ? round($row->ps_hi / $row->pi_hi, 2) : 'N/A';
            })
            ->addColumn('ps_re_tot', function ($row) {
                return $row->re_tot != 0 ? round($row->ps_tot / $row->re_tot, 2) : 'N/A';
            })
            ->addColumn('ps_pi_tot', function ($row) {
                return $row->pi_tot != 0 ? round($row->ps_tot / $row->pi_tot, 2) : 'N/A';
            })
            ->make(true);
    }
}
